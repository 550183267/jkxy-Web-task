	...
	$.ajax({
		...
		success: function(msg5) {
			...
			$.each(tabe, function(index, array) { //遍历json数据列
				tb += "<tr><td>" + array['id'] + " </td ><td> " + array['category'] + "</td ><td> " + array['title'] + "</td ><td> " + array['pic'] + "</td ><td> " + array['time'] + "</td ><td><button name='" + array['id'] + "' ' class='edit' id='editnews'>编辑</button></td><td><button name='" + array['id'] + "' class='del'>删除</button></td></tr>";
			});
			$("#tbody").append(tb);
